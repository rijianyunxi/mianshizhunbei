...
function sanitizeMessages(messages, normalizeRole) {
  if (!Array.isArray(messages)) return [];
  const sanitized = [];
...
function buildSlidingWindow(messages, { normalizeRole, systemPrompt, contextRounds }) {
  const normalized = sanitizeMessages(messages, normalizeRole);
  ...
  const keepCount = Math.max(2, contextRounds * 2);
  const recent = nonSystem.slice(-keepCount);

  const systemMessage = firstSystem || { role: 'system', content: systemPrompt };
...
function assertFunction(name, value) {
  if (typeof value !== 'function') {
    throw new Error(`smart-construction-agent-core missing dependency: ${name}`);
  }
}
...
export function createSmartConstructionAgentService(deps = {}) {
  const {
  ...
  assertFunction('DynamicStructuredTool', DynamicStructuredTool);
  assertFunction('createAgent', createAgent);
  assertFunction('ChatOpenAI', ChatOpenAI);
  assertFunction('uuidv4', uuidv4);
  assertObject('z', z);
  assertObject('env', env);
  assertObject('mcpRegistry', mcpRegistry);
  assertFunction('clearThreadCheckpoints', clearThreadCheckpoints);
  assertObject('toolRouter', toolRouter);
  assertFunction('jsonSchemaToZod', jsonSchemaToZod);
  assertFunction('normalizeRole', normalizeRole);
  assertFunction('splitText', splitText);
  ...
  class SmartConstructionAgentService {
    ...
    async buildRuntime(rawInput) {
      ...
      const selectedToolsRaw = await toolRouter.selectTools(query, env.TOOL_ROUTER_TOP_K);
      const selectedTools = Array.isArray(selectedToolsRaw) ? selectedToolsRaw : [];
  ...
  return new SmartConstructionAgentService();
}
